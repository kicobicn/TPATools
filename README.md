...
- set
1. /tpatool setlanguage [en_us/zh_cn] - change mod language
2. /tpatool setmaxhome [count] - Set the maximum number of homes
3. /tpatool needop [tpa/home/grave/back] - changing the selected command require op or not
4. /tpatool tpawaittime [time] (to set the required timeout duration)
5. /tpatool tpacdtime [time] (for setting cooldown time)
6. /tpatool debug [true/false] - to switch debug log (add in 1.0.6)
...
- 设置
1. /tpatool setlanguage [en_us/zh_cn] 更改mod语言
2. /tpatool setmaxhome [count] 设置最大家的数量
3. /tpatool needop [tpa/home/grave/back] 设置指定指令是否需要op
4. /tpatool tpawaittime [time] (用于设置超时所需时间)
5. /tpatool tpacdtime [time]（用于设置冷却时间）
6. /tpatool debug [true/false] (默认关闭) （于1.0.6添加）
